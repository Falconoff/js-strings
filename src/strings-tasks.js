...
function removeFirstOccurrences(str, value) {
  ...
  const firstIndexOfValue = str.indexOf(value);
  // if not exist substring in string
  if (firstIndexOfValue < 0) {
    return str;
  }
  const valueLength = value.length;
  const lastIndexOfValue = firstIndexOfValue + valueLength;
  return str.slice(0, firstIndexOfValue) + str.slice(lastIndexOfValue);
}
...
function isPalindrome(str) {
  // RegExp /[\W_]/g is equivalent to [^A-Za-z0–9_] to remove all non-alphanumeric characters (punctuation, spaces and symbols)
...
function reverseWords(str) {
  ...
  const reversedWords = arrFromString.map((word) =>
...
function encodeToRot13(str) {
  ...
  str.split('').map((char) => {
    const ind13 = originalStr.indexOf(char);
